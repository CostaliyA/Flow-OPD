...
def compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config):
    """Original function: policy only, uses disable_adapter for base-only reference."""
    if config.train.cfg:
...
def main(_):
    ...
    # Load LoRA+base reference transformer for KL reward (policy vs LoRA+base)
    ref_pipeline = StableDiffusion3Pipeline.from_pretrained(
        config.pretrained.model
    )
    ref_pipeline.transformer.requires_grad_(False)
    ref_pipeline.transformer.to(accelerator.device)
    # ref_transformer: used as LoRA+base reference for KL reward
    # Load LoRA from kl_ref_lora_path onto the base model
    if config.train.get("kl_ref_lora_path"):
        ...
        ref_transformer.set_adapter("default")
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        if reward_mode == "kl_only":
                            ...
                            if config.train.get("kl_reward_level") == "step_wise" and config.train.get("kl_scale", 0) != 0:
                                ...
                                kl_reward = kl_reward.squeeze(-1).squeeze(-1)  # [batch_size], keep per-sample variation across batch

                                kl_norm = config.train.get("kl_norm", "none")
                                if kl_norm == "per_sample":
                                    # Normalize across timesteps per sample (not applicable since we already removed timestep dim)
                                    kl_mean = kl_reward.mean()
                                    kl_std = kl_reward.std() + 1e-4
                                    kl_reward = (kl_reward - kl_mean) / kl_std
                                elif kl_norm == "per_timestep":
                                    # Normalize across samples at each timestep
                                    kl_mean = kl_reward.mean()
                                    kl_std = kl_reward.std() + 1e-4
                                    kl_reward = (kl_reward - kl_mean) / kl_std
                                elif kl_norm == "global":
                                    # Normalize globally across samples
                                    kl_mean = kl_reward.mean()
                                    kl_std = kl_reward.std() + 1e-4
                                    kl_reward = (kl_reward - kl_mean) / kl_std
                            ...
                            task_advantage = sample["advantages"][:, last_step_index]  # same for all steps, just use step-0 index

                            kl_norm = config.train.get("kl_norm", "none")
                            if kl_norm == "per_sample":
                                adv_mean = task_advantage.mean()
                                adv_std = task_advantage.std() + 1e-4
                                task_advantage = (task_advantage - adv_mean) / adv_std
                            elif kl_norm == "per_timestep":
                                adv_mean = task_advantage.mean()
                                adv_std = task_advantage.std() + 1e-4
                                task_advantage = (task_advantage - adv_mean) / adv_std
                            ...
                            # Step-wise KL reward computed at every step
                            if config.train.get("kl_reward_level") == "step_wise" and config.train.get("kl_scale", 0) != 0:
                                kl_reward = ((prev_sample_mean - prev_sample_mean_ref_lora) ** 2).mean(dim=(1, 2, 3), keepdim=True) / (2 * std_dev_t ** 2)
                                kl_reward = kl_reward.squeeze(-1).squeeze(-1)  # [batch_size], keep per-sample variation across batch
